...
// does not match any tiles, we should display a temporary notification
// instead of just doing nothing

function haveSnack(text,bgcolor,showClass = 'snackbar',duration = 3000) {
    let toast = document.createElement('div');
    toast.className = showClass;
    toast.innerText = text;
    // may need to generate a unique ID if its possible to have multiple such events
    toast.id = 'zeronotify';
    toast.style.backgroundColor = bgcolor;
    toast.style.zIndex = 800;
    // it looks a little glitchy if we add visible element and trigger animation
